...
function TableHeader({ className, ...props }: React.HTMLAttributes<HTMLTableSectionElement>) {
  return <thead className={cn("bg-muted sticky-thead [&_tr]:border-b", className)} {...props} />;
}
...
function TableHead({ className, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) {
  return (
    <th
      className={cn(
        "h-11 px-4 text-left align-middle font-semibold text-foreground/80 text-xs uppercase tracking-wide whitespace-nowrap",
        className
      )}
      {...props}
    />
