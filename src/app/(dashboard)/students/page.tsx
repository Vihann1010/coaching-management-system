...
import { UserPlus, Users as UsersIcon, Download, Phone } from "lucide-react";
...
import { FeeStatusBadge, ActiveStatusBadge } from "@/components/shared/status-badge";
...
export default async function StudentsPage({
  ...
  const [{ data: batches }, studentsQuery] = await Promise.all([
  ...
  return (
    <div>
      <PageHeader
        title="Students"
        description={`${total} student${total === 1 ? "" : "s"} ${statusFilter === "active" ? "active" : statusFilter === "inactive" ? "inactive" : ""}`}
        actions={
          <>
